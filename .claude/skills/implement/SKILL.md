...
1. **Identify the active feature**:
   ...
   - Fail fast if the feature has no tasks yet (`list_tasks` returns empty): tell the user to run `/tasks` first.
...
7. **Execute implementation** following the task plan:
   ...
   - **When you start the first task in a phase that maps to a user story** (i.e., `phase.user_story_id` is set), flip that user_story to `'in_progress'`:
     ```
     mcp__specs-mcp__update_entity(
       kind='user_story', code_or_id=<phase.user_story_id>,
       fields={'status': 'in_progress'}
     )
     ```
     Idempotent — fine to run on every task pick.
   ...
   - **After marking a task `done`, check whether its phase is now complete.** If the phase has `user_story_id` and zero non-`done` tasks remain in it, flip the user_story to `'done'`:
     ```
     mcp__specs-mcp__execute_sql(
       query="SELECT COUNT(*) AS pending FROM task WHERE phase_id = ? AND status != 'done'",
       params=[<phase_id>]
     )
     # if pending == 0:
     mcp__specs-mcp__update_entity(
     ...
     ```
     (`'verified'` is a stronger state that requires an independent verifier — Constitution Principle III. Not handled by this skill yet; `'done'` is the practical terminus until a verifier sub-agent is wired in.)
...
10. **Completion validation**:
    ...
    - **Verify every user story is `done`** (the per-phase update in step 7 should have handled this, but check the invariant):
      ```
      mcp__specs-mcp__execute_sql(
        query="SELECT ordinal, title, status FROM user_story "
              "WHERE feature_id = ? AND status != 'done'",
        params=[<feature_id>]
      )
      # Empty result = all stories done. Non-empty = something missed —
      # don't mark the feature done; investigate.
      ```
    ...
    - Mark the feature `done`:
      ...
      mcp__specs-mcp__update_entity(kind='feature', code_or_id='<slug>',
                                     fields={'status': 'done'})
      ```
    - Report final summary: tasks done / skipped / superseded, user_stories done / not-done, plus duration.

Note: This command assumes a complete task graph exists in the DB. If `list_tasks` returns empty, run `/tasks` first.
