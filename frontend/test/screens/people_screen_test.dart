...
void main() {
  group('PeopleScreen', () {
    ...
    testWidgets(
        'FR-001: initial render shows exactly one empty name field, Add another with + icon, and disabled Continue',
        (tester) async {
    ...
    testWidgets(
        'FR-001: tapping Add another appends a new empty name field with an ✕ remove button',
        (tester) async {
      await tester.pumpWidget(buildSubject());
      expect(find.byType(TextField), findsOneWidget);
    ...
    testWidgets(
        'FR-001: tapping ✕ removes that added field; the original first field never has an ✕',
        (tester) async {
      await tester.pumpWidget(buildSubject());
    ...
    testWidgets(
        'FR-011: Continue is enabled iff ≥2 trimmed non-empty fields; duplicates do NOT disable it',
        (tester) async {
      await tester.pumpWidget(buildSubject());
    ...
    testWidgets(
        'FR-017: tapping Continue with duplicates shows the indicator AND does not navigate AND does not commit state',
        (tester) async {
      await tester.pumpWidget(buildSubject());
    ...
    testWidgets(
        'FR-017: empty or whitespace-only fields disable Continue without showing any indicator; duplicate indicators do not appear during typing',
        (tester) async {
      await tester.pumpWidget(buildSubject());
    ...
    testWidgets(
        'FR-008: Continue with unique trimmed non-empty names commits to bill state in order and triggers navigation',
        (tester) async {
      await tester.pumpWidget(buildSubject());
