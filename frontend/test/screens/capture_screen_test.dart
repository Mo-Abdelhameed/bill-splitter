...
void main() {
  group('CaptureScreen', () {
    ...
    testWidgets(
        'FR-002: initial render shows Take Photo + Pick from Gallery, no preview, no permission error, leaves billState.imageBytes unchanged',
        (tester) async {
    ...
    testWidgets(
        'FR-002: tapping Take Photo with camera permission granted invokes acquirer.takePhoto',
        (tester) async {
    ...
    testWidgets(
        'FR-002: tapping Pick from Gallery with photo-library permission granted invokes acquirer.pickFromGallery',
        (tester) async {
    ...
    testWidgets(
        'FR-002: a returned image is resized with maxLongEdge=1600, stored in billState.imageBytes, and the screen enters the preview state',
        (tester) async {
    ...
    testWidgets(
        'FR-002: preview state renders an Image, plus Use this photo and Retake or repick buttons; original CTAs are hidden',
        (tester) async {
    ...
    testWidgets(
        'FR-002: tapping Use this photo triggers navigation (onContinue) without modifying billState.imageBytes',
        (tester) async {
      acquirer.takeResult = originalBytes;
      await tester.pumpWidget(buildSubject());
      await tester.tap(find.text('Take Photo'));
    ...
    testWidgets(
        'FR-002: tapping Retake or repick clears billState.imageBytes to null and returns to the initial 2-CTA state',
        (tester) async {
      acquirer.takeResult = originalBytes;
      await tester.pumpWidget(buildSubject());
      await tester.tap(find.text('Take Photo'));
    ...
    testWidgets(
        'FR-002: cancelling the picker (null result) shows a "No image selected" snackbar and keeps the screen in the initial state with billState unchanged',
        (tester) async {
    ...
    testWidgets(
        'FR-002: camera permission denied shows an inline camera-denied error with an Open Settings button; Pick from Gallery still works',
        (tester) async {
    ...
    testWidgets(
        'FR-002: photo-library permission denied shows an inline gallery-denied error with Open Settings; Take Photo still works',
        (tester) async {
